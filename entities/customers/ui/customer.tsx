...
import type { Customer as CustomerModel } from "@/entities/customers/model/customers-slice"
...
export function Customer({ customer }: CustomerProps) {
  ...
  useFrame((state, delta) => {
    ...
    if (isWalking.current) {
      ...
      if (leftLegRef.current) leftLegRef.current.rotation.x = THREE.MathUtils.lerp(
        leftLegRef.current.rotation.x, 0, delta * 5
      )
      if (rightLegRef.current) rightLegRef.current.rotation.x = THREE.MathUtils.lerp(
        rightLegRef.current.rotation.x, 0, delta * 5
      )
    }
  })
