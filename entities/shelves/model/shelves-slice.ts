...
export type ShelfType = "dairy" | "bakery" | "produce" | "meat" | "vegetables"
...
export const shelvesSlice = createSlice({
  ...
  reducers: {
    ...
    updateShelfPosition: (
      state,
      action: PayloadAction<{ id: string; position: { x: number; y: number; z: number } }>,
    ) => {
    ...
    updateShelfRotation: (state, action: PayloadAction<{ id: string; rotation: number }>) => {
...
export const { addShelf, updateShelfPosition, updateShelfRotation, incrementInteraction, selectShelf, removeShelf, clearShelves, setShelves } =
